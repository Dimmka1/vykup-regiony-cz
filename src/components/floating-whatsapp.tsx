...
const WHATSAPP_PHONE = "420800123001";

function buildWhatsAppUrl(regionName: string): string {
  const text = `Dobrý den, mám zájem o výkup nemovitosti v ${regionName}.`;
...
export function FloatingWhatsApp({ regionName }: FloatingWhatsAppProps) {
  const href = buildWhatsAppUrl(regionName);

  const handleClick = () => {
    trackEvent("whatsapp_click", { region: regionName });
  };
